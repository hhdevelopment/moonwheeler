...
import {Attribute, ChangeDetectionStrategy, ChangeDetectorRef, Component, ElementRef, EventEmitter, forwardRef, HostListener, Inject, Input, OnDestroy, OnInit, Optional, Output, ViewChild, ViewEncapsulation} from '@angular/core';
import {ControlValueAccessor, NG_VALUE_ACCESSOR} from '@angular/forms';
import {CanColor, CanColorCtor, CanDisable, CanDisableCtor, HammerInput, HasTabIndex, HasTabIndexCtor, mixinColor, mixinDisabled, mixinTabIndex} from '@angular/material/core';
 ...
 */
export const APP_SLIDER_VALUE_ACCESSOR: any = {
...
// tslint:disable-next-line:variable-name
export const _AppSliderMixinBase:
  HasTabIndexCtor &
  CanColorCtor &
  CanDisableCtor &
  typeof SliderBase =
  mixinTabIndex(mixinColor(mixinDisabled(SliderBase), 'accent'));
...
//  moduleId: module.id,
  selector: 'app-slider',
  exportAs: 'SliderComponent',
  providers: [APP_SLIDER_VALUE_ACCESSOR],
...
export class SliderComponent extends _AppSliderMixinBase
  ...
  get _trackBackgroundStyleLeftTransform(): string {
    const axis = this.vertical ? 'Y' : 'X';
    let percent: number;
    if (this.percent instanceof Array) {
    ...
    const sign = this._shouldInvertMouseCoords() ? '-' : '';
    return 'inherit'; // `translate${axis}(${sign}0px) scale3d(${scale})`;
  }

  get _trackBackgroundStyleLeftWidth(): string {
    const axis = this.vertical ? 'Y' : 'X';
  ...
  get _trackBackgroundStyleRightTransform(): string {
    const axis = this.vertical ? 'Y' : 'X';
    let percent: number;
    if (this.percent instanceof Array) {
      percent = this.percent[0];
    ...
    const sign = this._shouldInvertMouseCoords() ? '' : '-';
    return `translate${axis}(${sign}0px) scale3d(${scale})`;
  }
  ...
  get _trackFillStyles(): { [key: string]: string } {
    ...
    let scale: string;
    if (this.percent instanceof Array) {
      scale = this.vertical ? `1, ${this.percent[1] - this.percent[0]}, 1` : `${this.percent[1] - this.percent[0]}, 1, 1`;
    } else {
      scale = this.vertical ? `1, ${this.percent}, 1` : `${this.percent}, 1, 1`;
    }
    const invertOffset = (this._getDirection() === 'rtl' && !this.vertical) ? !this._invertAxis : this._invertAxis;
  ...
  get _trackFillStyleTransform(): string {
    const axis = this.vertical ? 'Y' : 'X';
    const invertOffset = (this._getDirection() === 'rtl' && !this.vertical) ? !this._invertAxis : this._invertAxis;
    let offset = 0;
    let percent: number;
    if (this.percent instanceof Array) {
      offset = (invertOffset ? 1 - this.percent[1] : this.percent[0]) * 100;
      percent = this.percent[1] - this.percent[0];
    } else {
      percent = this.percent;
    }
    const scale = this.vertical ? `1, ${percent}, 1` : `${percent}, 1, 1`;
    ...
    let result: string;
    if (this.percent instanceof Array) {
      result = `translate${axis}(${offset}%) scale3d(${scale})`;
    } else {
      result = `translate${axis}(${sign}${this._thumbGap}px) scale3d(${scale})`;
    }
    return result;
  }
  ...
  get _ticksContainerStyleTransform(): string {
    const axis = this.vertical ? 'Y' : 'X';
    // For a horizontal slider in RTL languages we push the ticks container off the left edge
    // instead of the right edge to avoid causing a horizontal scrollbar to appear.
    const sign = !this.vertical && this._getDirection() === 'rtl' ? '' : '-';
    const offset = this._tickIntervalPercent / 2 * 100;
    return `translate${axis}(${sign}${offset}%)`;
  }
  ...
  get _ticksStyleBgSize(): string {
    const tickSize = this._tickIntervalPercent * 100;
    const result = this.vertical ? `2px ${tickSize}%` : `${tickSize}% 2px`;
    return result;
  }

  get _ticksStyleTransform(): string {
    const tickSize = this._tickIntervalPercent * 100;
    const axis = this.vertical ? 'Y' : 'X';
    const sign = !this.vertical && this._getDirection() === 'rtl' ? '-' : '';
    const rotate = !this.vertical && this._getDirection() === 'rtl' ? ' rotate(180deg)' : '';
    return `translateZ(0) translate${axis}(${sign}${tickSize / 2}%)${rotate}`;
  }

  get _ticksStylePaddingBottom(): string {
    return this._ticksStylePadding('Bottom');
  }

  get _ticksStylePaddingTop(): string {
    return this._ticksStylePadding('Top');
  }

  get _ticksStylePaddingRight(): string {
    return this._ticksStylePadding('Right');
  }

  get _ticksStylePaddingLeft(): string {
    return this._ticksStylePadding('Left');
  }

  _ticksStylePadding(dir: 'Bottom' | 'Top' | 'Right' | 'Left'): string {
    if (this._isMinValue && this._thumbGap) {
      const side = this.vertical ? (this._invertAxis ? 'Bottom' : 'Top') : (this._invertAxis ? 'Right' : 'Left');
      if (side === dir) {
        return `${this._thumbGap}px`;
      }
    }
    return 'inherit';
  }
  ...
  get _thumbContainerStyleLeftTransform(): string {
    const axis = this.vertical ? 'Y' : 'X';
    // For a horizontal slider in RTL languages we push the thumb container off the left edge
    // instead of the right edge to avoid causing a horizontal scrollbar to appear.
    const invertOffset = (this._getDirection() === 'rtl' && !this.vertical) ? !this._invertAxis : this._invertAxis;
  ...
  get _thumbContainerStyleRightTransform(): string {
    const axis = this.vertical ? 'Y' : 'X';
    // For a horizontal slider in RTL languages we push the thumb container off the left edge
    // instead of the right edge to avoid causing a horizontal scrollbar to appear.
    const invertOffset = (this._getDirection() === 'rtl' && !this.vertical) ? !this._invertAxis : this._invertAxis;
  ...
  _onClick(event: MouseEvent) {
    ...
    this._manageEventEmitting(oldValue);
  }

  _manageEventEmitting(oldValue: any) {
  ...
  @HostListener('slidestart', ['$event', 'dir'])
